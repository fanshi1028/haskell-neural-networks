...
getActivation' Sigmoid = \x dY ->
  let z = getActivation Sigmoid x
      ones = (rows z >< cols z) $ repeat 1
   in z * (ones - z) * dY
getActivation' Relu = \x dY -> cmap (\z -> if z >= 0 then 1 else 0) x * dY
getActivation' Tanh = \x dY ->
  let z = getActivation Tanh x
      ones = (rows z >< cols z) $ repeat 1
   in (ones - cmap (^ 2) z) * dY
